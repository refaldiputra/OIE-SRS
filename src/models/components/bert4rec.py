...
class BERT4RecModel(SequentialRecModel):
    def __init__(self,
        ...
        super(BERT4RecModel,self).__init__(item_size, hidden_size, max_seq_length, initializer_range)

        # load parameters info
        self.mask_ratio = mask_ratio
        self.item_embeddings = nn.Embedding(item_size+1, self.hidden_size, padding_idx=0) # item_size + 1 because of the mask token

        # load dataset info
        self.mask_token = item_size
        self.n_items = item_size
        self.mask_item_length = int(self.mask_ratio * max_seq_length)

        # define layers and loss
        self.item_encoder = TransformerEncoder(transformer)
    ...
    def multi_hot_embed(self, masked_index, max_length):
        ...
        multi_hot = torch.zeros(
            masked_index.size(0), max_length
        )
    ...
    def calculate_loss(self, input_ids, answers, neg_answers, same_target, user_ids):
        ...
        pred_index_map = pred_index_map.view(masked_index.size(0), masked_index.size(1), -1).to(seq_output.device)  # [B mask_len max_len]
        # [B mask_len max_len] * [B max_len H] -> [B mask_len H]
        # only calculate loss for masked position
        # print(pred_index_map.device, seq_output.device)
